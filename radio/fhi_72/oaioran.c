  ...
  for (uint16_t cc_id = 0; cc_id < 1 /*nSectorNum*/; cc_id++) { // OAI does not support multiple CC yet.
    for (int aa = ru->start_prach_rx; aa < ru->start_prach_rx + ru->nb_prach_rx; aa++) {
      for (sym_idx = prach_start_sym; sym_idx < prach_end_sym; sym_idx++) {
        ...
        int min_prb = p_rx_packet_ctl->nRBStart[0];
        int max_prb = p_rx_packet_ctl->nRBStart[0] + p_rx_packet_ctl->nRBSize[0];
        for (int i = 1; i < nRxPkt; i++) {
          if (p_rx_packet_ctl->nRBStart[i] < min_prb) {
            min_prb = p_rx_packet_ctl->nRBStart[i];
          }
          if (p_rx_packet_ctl->nRBStart[i] + p_rx_packet_ctl->nRBSize[i] > max_prb) {
            max_prb = p_rx_packet_ctl->nRBStart[i] + p_rx_packet_ctl->nRBSize[i];
          }
        }
        int num_total_prbs = max_prb - min_prb;
        ...
        for (int pkt_idx = 0; pkt_idx < nRxPkt; pkt_idx++) {
          ...
          int16_t *decom_dst = local_dst + (start_prbu - min_prb) * N_SC_PER_PRB * 2;
