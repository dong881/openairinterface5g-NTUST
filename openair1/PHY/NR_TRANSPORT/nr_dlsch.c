...
void nr_generate_pdsch(PHY_VARS_gNB *gNB, int n_dlsch, NR_gNB_DLSCH_t *dlsch_array, int frame, int slot)
{
  ...
  for (int i = 0; i < n_dlsch; i++) {
    ...
    output_ptr += do_one_dlsch(output_ptr, gNB, &dlsch_array[i], slot, -1);
  }
}
...
int nr_pdsch_encoding_phase(PHY_VARS_gNB *gNB, int n_dlsch, NR_gNB_DLSCH_t *dlsch_array, int frame, int slot)
{
  ...
  for (int dlsch_id = 0; dlsch_id < n_dlsch; dlsch_id++) {
    NR_gNB_DLSCH_t *dlsch = &dlsch_array[dlsch_id];
    const nfapi_nr_dl_tti_pdsch_pdu_rel15_t *rel15 = &dlsch->pdsch_pdu->pdsch_pdu_rel15;
    const int Qm = rel15->qamModOrder[0];
  ...
  start_meas(dlsch_encoding_stats);
  if (slot_timing_enabled) clock_gettime(CLOCK_MONOTONIC, &t_enc_start);
  if (nr_dlsch_encoding(gNB, n_dlsch, dlsch_array, frame, slot, frame_parms, g_pdsch_encoded_output,
...
void nr_pdsch_codeword_phase(PHY_VARS_gNB *gNB, int n_dlsch, NR_gNB_DLSCH_t *dlsch_array, int frame, int slot)
{
  ...
  for (int dlsch_id = 0; dlsch_id < n_dlsch; dlsch_id++) {
    output_ptr += do_one_dlsch(output_ptr, gNB, &dlsch_array[dlsch_id], slot, dlsch_id);
  }
}
