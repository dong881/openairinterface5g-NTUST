...
// Log file parameters
// Set to 128MB, pre-allocate space
...
// Simplified cleanup function: no truncation, only synchronization
void cleanup_mmap_logger() {
  for (int i = 0; i < num_log_files; i++) {
    ...
    if (log->is_active) {
        ...
        if (log->log_fd != -1) {
          // Note: ftruncate has been removed here
          // The file will remain at 128MB size, with zeros filled later
          close(log->log_fd);
          log->log_fd = -1;
        }
