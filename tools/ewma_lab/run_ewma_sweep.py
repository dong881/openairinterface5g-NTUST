...
def run_cmd(command: str, *, check: bool = True, dry_run: bool = False) -> subprocess.CompletedProcess[str]:
...
def ssh(host: str, command: str, *, check: bool = True, dry_run: bool = False) -> subprocess.CompletedProcess[str]:
    return run_cmd(f"ssh {shlex.quote(host)} {shlex.quote(command)}", check=check, dry_run=dry_run)
...
def build_nr_softmodem(args: argparse.Namespace) -> None:
    ...
    if not args.skip_pnf_build:
        ssh(args.pnf_host, f"cd {quote_remote_path(args.pnf_build_dir)} && sudo ninja nr-softmodem", dry_run=args.dry_run)
...
def start_pnf(args: argparse.Namespace, alpha: int, beta: int, run_dir: Path) -> Path:
    ...
    pnf_command = (
        ...
        f"sudo NFAPI_TRACE_LEVEL=info ./nr-softmodem -O {shlex.quote(args.pnf_conf)} "
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--pnf-build-dir", default="~/openairinterface5g/cmake_targets/ran_build/build")
    parser.add_argument("--pnf-conf", default="../../../targets/PROJECTS/GENERIC-NR-5GC/CONF/gnb-pnf-split-direct.sa.band78.fhi72.nfapi.4x4-pegatron.conf")
