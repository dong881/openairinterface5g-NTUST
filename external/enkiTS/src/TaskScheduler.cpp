...
namespace enki
{
    ...
    // OAI 5G: Increase spin count to prevent worker threads from sleeping
    // This eliminates wake-up latency spikes (9ms -> <100us) at cost of CPU usage
    // Workers spin on dedicated cores (6,7,8,9,10,11,13,14) so CPU cost is acceptable
    static constexpr uint32_t gc_SpinCount               = UINT32_MAX;  // Never sleep
